...
- **🛡️ جودة الحزمة (Pack Quality Gates)** — بوابات جودة أوتوماتيكية عند توليد الحزمة: فلترة USPs الركيكة (كلمة واحدة مثل "Location") مع إعادة محاولة Gemini وبديل محلي، تصنيف خدمات التجميل/الجراحة التجميلية + إزالة التكرار (حد 6)، عدم إخراج أقسام فارغة للآراء والأسئلة الشائعة، اتساق اللغة في AGENTS (اللغة كما هي + ترتيب البناء لمواقع متعددة اللغات)، مشاهد صور مناسبة، وعداد **Pack health X/5** يتحقق من (USP، insights، خدمات دون تكرار، اتساق اللغة، وجود الهاتف)
...
### ☁️ المزامنة السحابية (اختياري — Firebase مشروعك الخاص)
مزامنة **History + Leads** بين المتصفح وتطبيق سطح المكتب في الوقت الفعلي (Realtime):
- **اختيارية بالكامل** — معطّلة افتراضيًا؛ الأداة تبقى serverless إن لم تُفعّل
- **تُزامَن البيانات فقط**: History و Leads — لا تُرسل مفاتيح API أو الإعدادات أبدًا
- **Firebase خاص بك**: أدخل API Key + Project ID + App ID ثم رمز مرور (≥ 8 أحرف) في ⚙ Settings
- **خصوصية**: رمز المرور لا يغادر جهازك — يُخزّن SHA-256 فقط ويشتق منه مسار المستند (openprompt/{hash}) في Firestore
- **لا يتعطل الاستخدام المحلي**: أي فشل سحابي (غير متصل / مفتاح خاطئ) → حالة 🔴 في ⚙ Settings فقط
- عند التفعيل: اتصال (Connect) → مستمع لحظي (onSnapshot) → دفع أول مرة فقط إذا لم تكن الوثيقة موجودة
