...
# 🟣 المرحلة 12.6 — Import Leads (CSV Template → History)

## 🎯 الهدف
جلب عملاء خارجيين (إعلانات، أدوات أخرى) إلى History عبر قالب CSV.

## ⏱️ الوقت: 10 دقائق | 📈 النجاح: 100%

## الميزات المُنجزة
- ✅ زر "⬆ Import" في شريط History
- ✅ Modal استيراد مع قالب CSV قابل للتنزيل
- ✅ `downloadTemplate()` — قالب بعمودين مثال (أسنان + تجميل)
- ✅ `parseCSV()` — محلل CSV يدعم الحقول بين علامات الاقتباس
- ✅ `importLeads()` — بناء entries مع source:'imported' + dedupe
- ✅ شارة "📥 Imported" الزرقاء في سجل التاريخ
- ✅ Open → Generate → Save لا يُنشئ تكرارًا

## ✅ معايير النجاح
- [ ] تنزيل القالب → فتح في Excel/Sheets مع أعمدة صحيحة
- [ ] ملء 3 صفوف → رفع → "Imported 3 (0 skipped)"
- [ ] إعادة رفع نفس الملف → "Imported 0 (3 skipped)"
- [ ] فاصلة داخل علامات اقتباس تُعامل صحيحًا
- [ ] فتح عميل مستورد → Generate → Save بدون تكرار

---

# 🔐 المرحلة 11 — Access Lock (Settings-managed session lock)

## 🎯 الهدف
قفل كلمة مرور اختياري يُدار من Settings. عند التفعيل: كل ميزة تتطلب كلمة مرور مرة واحدة لكل جلسة.

## ⏱️ الوقت: 15 دقيقة | 📈 النجاح: 100%

## الميزات المُنجزة
- ✅ قسم "Access Lock" في نافذة Settings مع تبديل ON/OFF
- ✅ `sha256()` عبر `crypto.subtle` — لا كلمات مرور نصية محفوظة
- ✅ `ensureUnlocked(action)` — بوابة لكل وظيفة رئيسية
- ✅ `lockGate(fn)` — مساعد عام لتغليف أي وظيفة
- ✅ نافذة إدخال كلمة المرور مع تأثير اهتزاز عند خطأ
- ✅ `sessionStorage` — كلمة المرور مرة واحدة لكل تبويب
- ✅ تغيير كلمة المرور يتطلب كلمة المرور الحالية
- ✅ `go(n≥1)`, `analyze`, `prospectSearch`, `generate`, `aiImprove`, `openInTool`, `exportAll` — جميعها مُقيّدة

## ✅ معايير النجاح
- [ ] تفعيل + تعيين كلمة مرور → حفظ
- [ ] تبويب جديد → نقر Extract → تظهر النافذة
- [ ] كلمة مرور خاطئة → اهتزاز + رسالة خطأ
- [ ] كلمة مرور صحيحة → المتابعة
- [ ] إعادة تحميل نفس التبويب → لا إعادة إدخال
- [ ] تعطيل القفل → جميع الميزات تعمل بدون نافذة
